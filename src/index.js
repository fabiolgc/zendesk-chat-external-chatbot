...
function connectWebSocket(websocket_url) {
  ...
  async function callExternalBot(chatMessage){
    try {
      const url = BOT_API_URL + "/" + BOT_API_BODY.BOT_ID + "/converse/fabio2"
      const body = `{"type": "text", "text": "${chatMessage.content}"}`;
     ...
     await request
        ...
        .then((res) => {
         ...
         console.log(JSON.stringify(res.body.responses));


          if (res.body.responses.find( ({ component }) => component === 'QuickReplies' )) {
                ...
                payload: {
                  query: `mutation {
                                    sendQuickReplies(
                                        ...
                                        msg: "Ask me something thats included in the choices. Let's try again.",
                                        quick_replies: [
                                            {
                                                action: {
                                                    value: "What is a Context?"
                                                },
                                                text: "What is a Context?"
                                            },
                                            {
                                                action: {
                                                    value: "What is Bot Memory?"
                                                },
                                                text: "What is Bot Memory?"
                                            }
                                        ],
                                        fallback: {
                                            msg: "Ask me something thats included in the choices. Let's try again."
          ...
          } else if (res.body.responses.find( ({ component }) => component === 'Dropdown' )) {

              console.log("dropdown");

              const sendListTemplateQuery = {
                payload: {
                  query: `mutation {
                                    sendListTemplate(
                                        channel_id: "${chatMessage.channel.id}",
                ...
                type: "request",
                id: REQUEST_ID.SEND_QUICK_TEMPLATES
              };

              webSocket.send(JSON.stringify(sendListTemplateQuery));




            }  else if (res.body.responses[item][subItem].type &&
                res.body.responses[item][subItem].type === "text") {
                ...
                const sendMessageQuery = {
                   payload: {
                     query: `mutation {
                                       sendMessage(
                                           channel_id: "${chatMessage.channel.id}",
                                           msg: "${res.body.responses[item][subItem].text}"
                                       ) {
                                           success
                                       }
                                   }`
        ...
        console.log("[callExternalBot] Request fail");
        console.log(err.response.error.text);
      }
  }
